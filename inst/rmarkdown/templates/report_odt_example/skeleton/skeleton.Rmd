...
######################################################################
use_package <- function(p) { # Rename this... Devtools has function by the same name
...
```{r dataset, echo = FALSE, results='asis', fig.cap='Caption here'}
pander(head(iris))
```
...
pander((1/z$scaling[1]) * z$scaling)
```

```{r confusion-mat, echo = FALSE, results = 'asis'}
pander(table(preds, iris_sub$Species))
```

Figures
-------

```{r irises, fig.width=6, fig.cap = "Three species of iris: I. Setosa, I. Virginica, I. Versicolor", echo = FALSE}
fig1 <- rasterGrob(readPNG("images/iris_setosa.png"), interpolate=TRUE)
fig2 <- rasterGrob(readPNG("images/iris_virginica.png"), interpolate=TRUE)
fig3 <- rasterGrob(readPNG("images/iris_versicolor.png"), interpolate=TRUE)
grid.arrange(fig1, fig2, fig3, ncol=3)
...
```{r pairsplot, echo=FALSE, fig.width=6, fig.height=5, fig.cap = "A matrix of scatterplots of the quantitative variables"}
par(mar = c(5,4,4,3))
pairs.panels(iris[,1:4],
             labels = c("Sepal Length",
                        "Sepal Width",
...
```{r histograms, fig.width = 6, fig.height=4, fig.cap = "Histograms of covariates separated by species", echo=FALSE, message = FALSE, error = FALSE, results = 'hide'}
plot1 <- ggplot(iris, aes(x=Sepal.Length))
plot1<- plot1 + geom_histogram(aes(fill = Species))
plot2 <- ggplot(iris, aes(x=Sepal.Width))
plot2<- plot2 + geom_histogram(aes(fill = Species))
...
```{r sep, fig.width = 6, fig.height=6, fig.cap = "Beanplots of the four quantitative variables by species", echo=FALSE}
par(mfrow = c(2,2))
beanplot(Sepal.Length ~ Species, data = iris, ylab = "Sepal Length",
         xlab = "Species", col = "beige", method = "jitter")
beanplot(Sepal.Width ~ Species, data = iris, ylab = "Sepal Width",
...
```{r scores, fig.width = 6, fig.height=6, fig.cap = "Linear discriminant scores for setosa and versicolor", echo=FALSE}
plot(z)
```
